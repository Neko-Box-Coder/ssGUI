    ...
    (
        [&]()
        {
                ...
                !inputInterface->IsInputExistLastFrame(ssGUI::Enums::SystemKey::ENTER))
            {
                ...
                ssLOG_LINE("menuItem.GetMinSize: "<<menuItem.GetMinSize().x<<", "<<menuItem.GetMinSize().y);
                ssLOG_LINE("menuItem.GetMaxSize: "<<menuItem.GetMaxSize().x<<", "<<menuItem.GetMaxSize().y);
                ssLOG_LINE("menu.GetMinSize: "<<menu.GetMinSize().x<<", "<<menu.GetMinSize().y);
                ssLOG_LINE("menu.GetMaxSize: "<<menu.GetMaxSize().x<<", "<<menu.GetMaxSize().y);
            }
        }
    );
